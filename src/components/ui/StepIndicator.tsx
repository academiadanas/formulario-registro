        ...
        <div className="flex justify-center items-center gap-1 sm:gap-1.5">
            {Array.from({ length: totalSteps }, (_, i) => i + 1).map((step) => (
                <div key={step} className="flex items-center gap-1 sm:gap-1.5">
                    <span
                        className={`w-8 h-8 sm:w-10 sm:h-10 rounded-full flex items-center justify-center font-semibold text-xs sm:text-[0.95rem]
              ...
              ${
                  step === currentStep
                    ...
                    {step < totalSteps && (
                        <span
                            className={`w-3 sm:w-6 h-0.5 rounded transition-all duration-300
